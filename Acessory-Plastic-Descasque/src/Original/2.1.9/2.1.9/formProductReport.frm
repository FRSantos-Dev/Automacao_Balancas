...
Public Sub Config()
   ...
   'CURSOR NO CLIENTE FAZ O RELATORIO SER EXIBIDO MAIS RÁPIDO
    ...
    If CRITERIO = "" Then
        RS.Open "SELECT referencia, data, IF(turno = 1, 'Noite', 'Manhã') AS iTurno, producao, MP_consumida, Nome, DATE_FORMAT(hora, GET_FORMAT(TIME, 'ISO')) AS HORA, tempo_injecao, ciclos, OBS FROM TBL_DADOS LEFT JOIN TBL_MP ON TBL_DADOS.COD_MP=TBL_MP.ID WHERE data BETWEEN '" & Format(DataIni, "yyyy/mm/dd") & "' AND '" & Format(DataFim, "yyyy/mm/dd") & "' ORDER BY referencia, data, turno", con
    Else
    ...
    Relatorio.Titulo = "Relatório de Produtos"
    Relatorio.Ativar
    RS.Close
    Unload Me
