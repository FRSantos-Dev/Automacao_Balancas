...
Begin VB.Form formRelaAPagar 
   ...
   End
End
Attribute VB_Name = "formRelaAPagar"
...
Public Sub Config(ByVal Valor As Integer, ByVal DataDe As Date, ByVal DataAte As Date)
    ...
    'CURS2OR NO CLIENTE FAZ O RELATORIO SER EXIBIDO MAIS RÁPIDO
    RS2.CursorLocation = adUseClient
    RS2.CursorType = adOpenStatic
    RS2.Open "SELECT T2.nome, T2.telefone, T2.endereco, T1.pagar, T1.Referencia, T1.peso_liq, T1.preco, T1.cod_desc, T1.data_saida FROM tbl_entrega AS T1 INNER JOIN tbl_descascador AS T2 ON T2.codigo = T1.cod_desc WHERE cod_desc = " & Valor & " and T1.data_dev <> 0 AND T1.quitado = 0 AND data_saida BETWEEN '" & Format(DataDe, "yyyy/mm/dd") & "' AND '" & Format(DataAte, "yyyy/mm/dd") & "'", con
...
Private Sub Relatorio_FormulaCampo(ByVal Campo As String, Valor As Variant)
    ...
    Select Case Campo
        Case "TotalPagar": Valor = pTotalPagar
    End Select
...
Private Sub Relatorio_ImprimiuRegistro(Cancelar As Boolean)
    ...
    pTotalPagar = pTotalPagar + Relatorio.Recordset!pagar
